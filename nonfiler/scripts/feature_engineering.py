...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["purchase_to_sales_ratio"] = df["ebm_tot_purchases"] / (
        df["ebm_tot_sales"] + 1e-6
    )
    df["vat_efficiency_ratio"] = df["ebm_vat_on_sales"] / (
        df["ebm_vat_on_purchases"] + 1e-6
    )

    df["sales_per_invoice"] = df["ebm_tot_sales"] / (df["invoice_no_sales"] + 1e-6)
    df["purchases_per_invoice"] = df["ebm_tot_purchases"] / (
        df["invoice_no_purchases"] + 1e-6
    )
    df["client_supplier_ratio"] = df["clients"] / (df["suppliers"] + 1e-6)

    # --- Group B: Proportions ---
    df["prop_taxable_sales"] = df["ebm_taxable_sales"] / (df["ebm_tot_sales"] + 1e-6)
    df["prop_exempted_purchases"] = df["ebm_exempted_purchases"] / (
        df["ebm_tot_purchases"] + 1e-6
    )
    df["prop_taxable_purchase"] = df["ebm_taxable_purchases"] / (
        df["ebm_tot_purchases"] + 1e-6
    )
    ...
    df["high_sales_flag"] = (
        df["ebm_tot_sales"] > df["ebm_tot_sales"].quantile(0.9)
    ).astype(int)
    ...
    df["sales_gt_purchase_flag"] = (
        df["ebm_tot_sales"] > df["ebm_tot_purchases"]
    ).astype(int)

    df["diff_vat_sales_purchases"] = df["ebm_vat_on_sales"] - df["ebm_vat_on_purchases"]
    df["vat_sales_gt_purchases_flag"] = (
        df["ebm_vat_on_sales"] > df["ebm_vat_on_purchases"]
    ).astype(int)

    df["diff_taxable_sales_total"] = df["ebm_taxable_sales"] - df["ebm_tot_sales"]
    df["taxable_sales_gt_total_flag"] = (
        df["ebm_taxable_sales"] > df["ebm_tot_sales"]
    ).astype(int)

    df["diff_taxable_purchases_total"] = (
        df["ebm_taxable_purchases"] - df["ebm_tot_purchases"]
    )
    df["taxable_purchases_gt_total_flag"] = (
        df["ebm_taxable_purchases"] > df["ebm_tot_purchases"]
    ).astype(int)

    df["diff_sales_inclusive_exclusive"] = (
        df["ebm_tot_sales"] - df["ebm_total_sales_exclusive"]
    )
    df["inclusive_sales_gt_exclusive_flag"] = (
        df["ebm_tot_sales"] > df["ebm_total_sales_exclusive"]
    ).astype(int)

    df["diff_purchases_inclusive_exclusive"] = (
        df["ebm_tot_purchases"] - df["ebm_total_purchases_exclusive"]
    )
    df["inclusive_purchases_gt_exclusive_flag"] = (
        df["ebm_tot_purchases"] > df["ebm_total_purchases_exclusive"]
    ).astype(int)
    ...
    new_cols = pd.DataFrame(
        {
            "prop_exempted_sales": df["ebm_exempted_sales"]
            / (df["ebm_tot_sales"] + 1e-6),
            "prop_zero_rated_sales": df["ebm_zero_rated_sales"]
            / (df["ebm_tot_sales"] + 1e-6),
            "prop_zero_rated_purchases": df["ebm_zero_rated_purchases"]
            / (df["ebm_tot_purchases"] + 1e-6),
        }
    )

    # Add flags based on new proportions
    new_cols["high_exempted_sales_flag"] = (
        new_cols["prop_exempted_sales"] > 0.5
    ).astype(int)
    new_cols["high_zero_rated_sales_flag"] = (
        new_cols["prop_zero_rated_sales"] > 0.5
    ).astype(int)
    new_cols["high_zero_rated_purchases_flag"] = (
        new_cols["prop_zero_rated_purchases"] > 0.5
    ).astype(int)
