...
def drop_highly_correlated_features(data, target_column=None, high_threshold=0.85):
    """
    ...
    if target_column:
        corr_matrix = numerical_data.drop(columns=[target_column], errors='ignore').corr()
    else:
    ...
    very_strong_corr = corr_pairs[abs(corr_pairs) > high_threshold].sort_values(ascending=False)
    ...
    corr_with_target = {}
    if target_column:
        corr_with_target = numerical_data.corrwith(numerical_data[target_column]).to_dict()
    ...
    return cleaned_data, sorted(features_to_drop_high)